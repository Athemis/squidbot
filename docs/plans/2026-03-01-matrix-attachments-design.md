...
squidbot/adapters/channels/cli.py     ← no change (receives str always)
squidbot/adapters/channels/email.py   ← no change (receives str always)
...
Matrix channel iterates the list and uploads/sends each file as a separate media event before the text reply.

CLI and Email channels ignore the attachment field (no change needed — they already only use `.text`).
...
MatrixChannel._handle_media(room, event)
  ↓
MatrixChannel._download_attachment(event) → (local_path: Path, multimodal_content: list[dict] | None)
  ↓
  if image MIME + size ≤ limit:
...
A configurable `max_inbound_media_bytes` field is added to `MatrixChannelConfig` (default: 10 MB). Images larger than this limit are passed as text-path only (no Base64 embedding). This matches nanobot-redux's `maxMediaBytes` concept.

## Error Handling

| Failure | Behavior |
|---|---|
| Image download fails | Text `[Anhang nicht verfügbar: ...]` |
| Base64 encoding error | Text-path fallback (no embedding) |
| Image > size limit | Text-path fallback |
...
- `test_download_attachment_non_image_returns_text_only`: mock download → no multimodal_content
- `test_download_attachment_too_large_returns_text_only`: oversized image → text fallback
- `test_send_attachment_uses_bytesio`: mock upload → assert BytesIO called (not lambda)
- `test_send_multiple_attachments`: two paths → two upload calls
- `test_inbound_message_multimodal_content`: InboundMessage field present and correct
